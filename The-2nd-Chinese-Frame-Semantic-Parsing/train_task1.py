...
def eval(model, val_loader):
    ...
    with torch.no_grad():
        for step, batch in tqdm(
            enumerate(val_loader), total=len(val_loader), desc="eval"
        ):
...
def train(model, train_loader, val_loader):
    ...
    for name, para in param_optimizer:
        model.state_dict()[name][:] += (
            (torch.rand(para.size()).to(device) - 0.5) * noise_lambda * torch.std(para)
        )

    # ***********************************************

    total_steps = int(
        len(train_loader) * args.num_train_epochs / args.accumulate_gradients
    )
        ...
        for step, batch in enumerate(iter_bar):
            ...
            if (step + 1) % args.accumulate_gradients == 0:
                lr_this_step = args.lr * warmup_linear(
                    global_step / total_steps, args.warmup_proportion
                )
...
def load_pretrained_bert(bert_model, init_checkpoint):
    if init_checkpoint is not None:
        ...
        if "model_bert_best" in init_checkpoint:
            ...
            state = {
                k.replace("bert.", "").replace("roformer.", ""): v
...
if __name__ == "__main__":
    ...
    train_dataset = Dataset(
        "./dataset/cfn-train.json", "./dataset/frame_info.json", tokenizer
    )
    dev_dataset = Dataset(
        "./dataset/cfn-dev.json", "./dataset/frame_info.json", tokenizer
    )
    ...
    # pretrain_model = BertModel.from_pretrained(args.pretrain_model_path)
    pretrain_model = BertModel.from_pretrained(args.pretrain_model_path)
    print(pretrain_model)
    model = Model(config, pretrain_model).to(device)
